...
export const Route = createFileRoute("/s/$siteSlug/$")({
  loader: async ({ params }) => {
    ...
    const result = await getPublishedPage({
      data: { siteSlug: params.siteSlug, path: path === "/" ? "/" : path },
    });
    if (!result) throw notFound();
    return { ...result, currentPath: path === "/" ? "/" : path };
  },
  head: ({ loaderData, params }) => {
    const name = loaderData?.site?.name ?? "Site";
    const pageTitle = loaderData?.page?.title;
    const title = loaderData?.page?.seoTitle?.trim() || (pageTitle ? `${pageTitle} — ${name}` : name);
    const description =
    ...
    return {
      ...
      meta: [
        ...
        { property: "og:url", content: `/s/${params.siteSlug}${loaderData?.currentPath ?? ""}` },
      ],
    };
  },
  notFoundComponent: () => (
