...
- **Phase 8-lite (AI builder)** done: `src/lib/ai.functions.ts` calls Lovable AI Gateway (`google/gemini-2.5-flash`, JSON mode) to generate full-page Puck drafts (allow-listed blocks only) and theme token patches. Every generation is stored in `ai_generations` with `status = pending`; apply snapshots the previous puckData into `page_versions` (source="ai"), overwrites, and records `ai_patch_applications` (before/after). Users can reject or roll back from a new dashboard at `/sites/$siteId/ai`.
- **Phase 9-lite (Hardening)** in progress: added `src/lib/sanitize.server.ts` (allow-list HTML sanitizer, deep `*Html`-prop walker), `src/lib/rate-limit.server.ts` (in-memory sliding-window limiter), `src/lib/audit.server.ts` (`logAudit` + `getRequestIp`). Sanitization now runs on `saveBlogPost.contentHtml`, `saveProduct.descriptionHtml`, and every AI-generated `*Html` prop before persistence. Public `submitForm` and `placeOrder` are rate-limited per IP. Privileged actions (blog publish/unpublish/delete, order status update, AI apply/rollback) write to a new `audit_logs` table, exposed at `/sites/$siteId/audit` (`src/routes/_authenticated/sites.$siteId.audit.index.tsx`) via `src/lib/audit.functions.ts`.
- **Phase 9-lite completed**: per-user AI rate limit (10 generations / 5 min, key `ai:<userId>`) on both `generatePagePatch` and `generateThemePatch`; audit coverage extended to `page.publish`, `page.revert`, `page.delete`, and `integration.create/update/delete` (credential values never logged — only a `credentialsRotated` flag).
- **Next up**: swap the bootstrap DDL for Drizzle Kit migrations, add Puck blocks that bind to blog/products/forms collections, and per-site analytics rollups.
